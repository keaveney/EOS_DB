...
CMD_LIST_COMPONENTS_BY_PROPERTY = '/cern-itkpd-test/' + TID + '-' + AWID + '/listComponentsByProperty'

INPUT_FILE = "./input/list_components_by_property.json"
OUTPUT_FLD = "./output"
...
def process_input_file(token):
  ...
  output_file = open(OUTPUT_FLD + "/list_components_by_property.json", "w")
...
def list_components(token, dto_in):
  ...
  url = CMD_GATEWAY + CMD_LIST_COMPONENTS_BY_PROPERTY
...
try:
    ...
    print("Welcome to the ITk Production Database! Retriving components by property value is being processed ...")
    print("")
